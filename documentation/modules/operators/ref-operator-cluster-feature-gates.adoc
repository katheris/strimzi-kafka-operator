...
The `UseConnectBuildWithBuildah` feature gate has a default state of _enabled_.
...
To disable the `UseConnectBuildWithBuildah` feature gate, specify `-UseConnectBuildWithBuildah` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.
endif::Section[]
...
[id='ref-operator-cert-manager-ca-type-feature-gate-{context}']
=== `CertManagerCaType` feature gate

The `CertManagerCaType` feature gate has a default state of _disabled_.

When enabled, Strimzi delegates issuing of TLS certificates to an external cert-manager instead of managing certificates internally.
With this feature gate enabled, you can set `type: cert-manager.io` on `clusterCa`, `clientsCa`, or both in the `Kafka` custom resource.
Strimzi then creates cert-manager `Certificate` resources and reads the CA public certificate from a user-provided `Secret` to establish TLS trust.
The Cluster Operator handles the Cluster CA and Clients CA end-entity certificates.
The User Operator handles per-`KafkaUser` certificates when `clientsCa.type` is set to `cert-manager.io`.

Before enabling this feature gate, apply the RBAC resources in `packaging/install/cert-manager/` and ensure that cert-manager is installed with a ready `Issuer` or `ClusterIssuer`.

To enable the `CertManagerCaType` feature gate, specify `+CertManagerCaType` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.

[id='ref-operator-use-background-pod-deletion-feature-gate-{context}']
=== `UseBackgroundPodDeletion` feature gate

The `UseBackgroundPodDeletion` feature gate has a default state of _disabled_.

When enabled, the Cluster Operator uses background deletion propagation when deleting pods during rolling updates, instead of the default foreground deletion propagation.

With foreground deletion, certain CNI plugins (such as Cilium) remove network endpoints immediately when a pod object is deleted.
This can cut off network access before broker containers complete their graceful shutdown, preventing them from communicating leadership relinquishment to controllers, sending final heartbeats, and completing other shutdown sequences.
The result is prolonged recovery times as the cluster re-establishes quorum.

With background deletion, the pod object is removed from the Kubernetes API immediately, but the underlying container runtime keeps the pod's network interfaces intact until all containers have fully exited.
This allows Kafka brokers to complete their graceful shutdown before network connectivity is removed.

Enable this feature gate if you are using a CNI plugin that aggressively removes network endpoints on foreground pod deletion and you are experiencing extended recovery times after rolling restarts.

To enable the `UseBackgroundPodDeletion` feature gate, specify `+UseBackgroundPodDeletion` in the `STRIMZI_FEATURE_GATES` environment variable in the Cluster Operator configuration.
