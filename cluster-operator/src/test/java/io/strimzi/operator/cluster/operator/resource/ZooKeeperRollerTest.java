...
public class ZooKeeperRollerTest {
    ...
    public void testAllPodsAreRolled(VertxTestContext context)  {
        ...
        when(leaderFinder.findZookeeperLeader(any(), any())).thenReturn(Future.succeededFuture(ZookeeperLeaderFinder.UNKNOWN_LEADER));

        MockZooKeeperRoller roller = new MockZooKeeperRoller(podOperator, leaderFinder, 300_00L);

        roller.maybeRollingUpdate(Reconciliation.DUMMY_RECONCILIATION, 3, DUMMY_SELECTOR, pod -> List.of("Should restart"))
    ...
    public void testNonReadyPodsAreRestartedFirst(VertxTestContext context) {
        ...
        ZookeeperLeaderFinder leaderFinder = mock(ZookeeperLeaderFinder.class);
        when(leaderFinder.findZookeeperLeader(any(), any())).thenReturn(Future.succeededFuture(leaderPodReady));

        MockZooKeeperRoller roller = new MockZooKeeperRoller(podOperator, leaderFinder, 300_00L);

        Function<Pod, List<String>> shouldRoll = pod -> List.of("Pod was manually annotated to be rolled");
        roller.maybeRollingUpdate(Reconciliation.DUMMY_RECONCILIATION, 3, DUMMY_SELECTOR, shouldRoll)
              .onComplete(context.succeeding(v -> context.verify(() -> {
                  assertThat(roller.podRestarts.size(), is(3));
    ...
    public void testNonReadinessOfPodCanPreventAllPodRestarts(VertxTestContext context)  {
        ...
        ZookeeperLeaderFinder leaderFinder = mock(ZookeeperLeaderFinder.class);
        when(leaderFinder.findZookeeperLeader(any(), any())).thenReturn(Future.succeededFuture(leaderPodNeedsRestart));

        MockZooKeeperRoller roller = new MockZooKeeperRoller(podOperator, leaderFinder, 300_00L);

        roller.maybeRollingUpdate(Reconciliation.DUMMY_RECONCILIATION, 3, DUMMY_SELECTOR, shouldRestart)
    ...
    public void testNonReadinessOfLeaderCanPreventAllPodRestarts(VertxTestContext context)  {
        ...
        ZookeeperLeaderFinder leaderFinder = mock(ZookeeperLeaderFinder.class);
        when(leaderFinder.findZookeeperLeader(any(), any())).thenReturn(Future.succeededFuture(leaderPodNeedsRestartNonReady));
    ...
    public void testNoPodsAreRolled(VertxTestContext context)  {
        ...
        roller.maybeRollingUpdate(Reconciliation.DUMMY_RECONCILIATION, 3, DUMMY_SELECTOR, pod -> null)
                .onComplete(context.succeeding(v -> context.verify(() -> {
                    assertThat(roller.podRestarts.size(), is(0));
    ...
    public void testLeaderIsLast(VertxTestContext context)  {
        ...
        ZookeeperLeaderFinder leaderFinder = mock(ZookeeperLeaderFinder.class);
        when(leaderFinder.findZookeeperLeader(any(), any())).thenReturn(Future.succeededFuture("name-zookeeper-1"));
