...
public class KafkaUserOperatorMockTest {
    ...
    private final UserCertIssuer caProvider = new InternalCaUserCertIssuer(new MockCertIssuer(), new PasswordGenerator(10, "a", "a"), null, Clock.systemUTC());
    ...
    public void testCreateTlsUser() throws ExecutionException, InterruptedException {
        ...
        KafkaUserOperator op = new KafkaUserOperator(ResourceUtils.createUserOperatorConfig(namespace), caProvider, secretOps, kafkaUserOps, scramOps, quotasOps, aclOps);
    ...
    public void testCreateTlsUserWithoutPkcs12() throws ExecutionException, InterruptedException {
        ...
        KafkaUserOperator op = new KafkaUserOperator(config, caProvider, secretOps, kafkaUserOps, scramOps, quotasOps, aclOps);
        CompletionStage<KafkaUserStatus> futureResult = op.reconcile(new Reconciliation("test-trigger", KafkaUser.RESOURCE_KIND, namespace, ResourceUtils.NAME), user, null);
        KafkaUserStatus status = futureResult.toCompletableFuture().get();
    ...
    public void testCreateTlsUserWithACLsDisabled() throws ExecutionException, InterruptedException {
        ...
        KafkaUserOperator op = new KafkaUserOperator(ResourceUtils.createUserOperatorConfig(namespace, Map.of(), false, "32", null), caProvider, secretOps, kafkaUserOps, scramOps, quotasOps, aclOps);
        CompletionStage<KafkaUserStatus> futureResult = op.reconcile(new Reconciliation("test-trigger", KafkaUser.RESOURCE_KIND, namespace, ResourceUtils.NAME), user, null);
        KafkaUserStatus status = futureResult.toCompletableFuture().get();
    ...
    public void testCreateScramShaUserWithConfigurableLength() throws ExecutionException, InterruptedException {
        ...
        KafkaUserOperator op = new KafkaUserOperator(ResourceUtils.createUserOperatorConfig(namespace, "30"), caProvider, secretOps, kafkaUserOps, scramOps, quotasOps, aclOps);
        CompletionStage<KafkaUserStatus> futureResult = op.reconcile(new Reconciliation("test-trigger", KafkaUser.RESOURCE_KIND, namespace, ResourceUtils.NAME), user, null);
        KafkaUserStatus status = futureResult.toCompletableFuture().get();
    ...
    public void testCreateTlsUserWithSecretPrefix() throws ExecutionException, InterruptedException {
        ...
        KafkaUserOperator op = new KafkaUserOperator(ResourceUtils.createUserOperatorConfig(namespace, Map.of(), true, "32", secretPrefix), caProvider, secretOps, kafkaUserOps, scramOps, quotasOps, aclOps);
        CompletionStage<KafkaUserStatus> futureResult = op.reconcile(new Reconciliation("test-trigger", KafkaUser.RESOURCE_KIND, namespace, ResourceUtils.NAME), user, null);
        KafkaUserStatus status = futureResult.toCompletableFuture().get();
    ...
    public void testReconciliationFailsWithDisabledAclOperator() {
        ...
        KafkaUserOperator op = new KafkaUserOperator(ResourceUtils.createUserOperatorConfig(namespace), caProvider, secretOps, kafkaUserOps, scramOps, quotasOps, new DisabledSimpleAclOperator());
        CompletionStage<KafkaUserStatus> futureResult = op.reconcile(new Reconciliation("test-trigger", KafkaUser.RESOURCE_KIND, namespace, ResourceUtils.NAME), user, null);
